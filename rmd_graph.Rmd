...
join1 <- full_join(q1_eg, q2_eg)
join2 <- full_join(join1, q3_eg)
all_sites <- full_join(join2, prm_eg) |> 
  mutate(no3_n = as.numeric(no3_n))
  
summary_all <- all_sites |> group_by(sample_id, month, year, na.rm = TRUE) |> summarize(mean_no3_n = mean(no3_n)) |> 
  mutate(date = as.yearmon(paste(summary_all$year, summary_all$month, "%Y %m")))

ggplot(data = summary_all, aes(x = year, y = mean_no3_n)) +
...
=======
```
>>>>>>> b44332e56746a8cb73efef55902d4a9ac5a553e5
